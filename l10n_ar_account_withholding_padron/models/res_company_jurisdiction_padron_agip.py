...
class ResCompanyJurisdictionPadronAgip(models.Model):
    _name = 'res.company.jurisdiction.padron.agip'
    ...
    def reescribirAlicuotaAGIP(self, contact=False):
        for rec in self:
            ...
            for line in find_lines:
                ...
                try:
                    ...
                    from_date = datetime.strptime(split_line[1], '%d%m%Y').date()
                    to_date = datetime.strptime(split_line[2], '%d%m%Y').date()
                    cuit = split_line[3]
                    alicuot = float(split_line[7].replace(',', '.'))
                    retention = split_line[8].replace(',', '.')
    ...
    def find_contacts_in_line(self, lines, contact=False):
        if not contact:
            ...
            cuits_datos = [line.split(';')[3] for line in lines]
            find_cuit = [cuit for cuit in cuit_array if cuit in cuits_datos]
            find_lines = [line for line in lines if line.split(';')[3] in find_cuit]
            return find_lines
        else:
